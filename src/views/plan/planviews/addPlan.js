...
import { handleAddPlan } from "../../../redux/actions/plan/add"
...
const AddPlan = () => {
  ...
  const { addPlanInProcess, success, error } = useSelector(
    (state) => state.plan
  )
  ...
  const onSubmit = (formData) => {
    if (isObjEmpty(errors)) {
      dispatch(handleAddPlan(formData))
      reset(formData)
    }
  }
